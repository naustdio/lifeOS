-- Per-ingredient estimated cost — proposal.md's originally-deferred `estimated_unit_cost` field.
-- Foundation for a future ShoppingList module (a shared cart would sum these across recipes), but
-- stands on its own today: the recipe detail can show an estimated total right now. Manual entry
-- only, per the module's existing "manual is fine" spirit — no price-lookup integration.
--
-- Only the INSERT ... values list inside the function bodies changes here, not either function's
-- own parameter list (`p_ingredients` stays one jsonb blob) — `create or replace` is safe.

alter table recipes.recipe_ingredients
  add column estimated_unit_cost numeric(10,2) check (estimated_unit_cost is null or estimated_unit_cost >= 0);
...
begin
  ...
  insert into recipes.recipes (household_id, owner_user_id, title, category, portions, video_url, prep_minutes, description)
  values (p_household_id, (select auth.uid()), p_title, p_category, coalesce(p_portions, 1), p_video_url, p_prep_minutes, p_description)
  ...
  loop
    ...
    insert into recipes.recipe_ingredients (recipe_id, position, name, quantity, unit, sub_recipe_id, estimated_unit_cost)
    values (v_recipe_id, (v_item->>'position')::int, v_item->>'name',
            nullif(v_item->>'quantity', '')::numeric, v_item->>'unit', v_sub_recipe_id,
            nullif(v_item->>'estimatedUnitCost', '')::numeric);
...
begin
  ...
  update recipes.recipes
     set title = p_title, category = p_category, portions = coalesce(p_portions, 1),
         video_url = p_video_url, prep_minutes = p_prep_minutes, description = p_description
