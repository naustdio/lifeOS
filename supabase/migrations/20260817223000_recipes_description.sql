-- Recipe description — UI-polish fast-follow (Penpot reference design showed a short paragraph
-- under the title). Plain column set by the write seam, same shape as every other core field
-- (title/category/portions/...); no separate direct-write function needed, unlike photo/favorite.

alter table recipes.recipes
  add column description text check (description is null or length(description) <= 500);

-- Parameter list shape changes (new trailing param, even defaulted) — `create or replace` alone
-- would leave the old 9-arg signature callable and blind to `description`. Drop it first.
drop function if exists recipes.create_recipe(uuid, text, text, int, text, jsonb, jsonb, text, int);
drop function if exists recipes.update_recipe(uuid, text, text, int, text, jsonb, jsonb, text, int);
