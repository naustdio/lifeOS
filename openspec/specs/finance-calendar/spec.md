...
The projection MUST include only recurring definitions with `active = true`. Paused definitions (`active = false`) MUST be excluded from every day cell in the window, and no manual future-dated transaction MUST ever be included. This projection covers `expense`-type definitions only — `transfer`-type auto-pay definitions (introduced later by `finance-credit-card-payments`) are excluded, consistent with this capability's expense-outflows-only scope.
...
#### Scenario: Transfer-type auto-pay definitions are excluded
- GIVEN a household has an active `transfer`-type recurring definition (e.g. credit-card auto-pay)
- WHEN the calendar is rendered
- THEN that definition never appears in any day cell and never reduces any day's projected balance
