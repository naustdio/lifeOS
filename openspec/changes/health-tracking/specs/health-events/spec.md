...
### Requirement: Four Costed Event Types
The system MUST support exactly four costed health event types: medical study/exam, medical consultation, medication/treatment, and vaccine. Logging an event of any of these types MUST post exactly one transaction to `finance.transactions` via the module-api seam with `origin_module = 'health'`.

#### Scenario: Each costed type posts a Finance transaction
- GIVEN a household member logs a health event of type medical study, consultation, medication, or vaccine, with a cost amount
...
- GIVEN a caller attempts to log a health event with a type outside the four costed types (and outside the non-costed vitals/profile records)
