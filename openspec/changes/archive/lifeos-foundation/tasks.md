...
> `design.md §N`.
>
> Tracker normalization (2026-08-04): tasks converted to canonical `- [x]` checkbox format for the
> native dispatcher. All 40 tasks (T-001..T-040) are delivered and marked complete; T-040's
> delivered-as-manual-checklist deviation is documented in its body (orchestrator-accepted
> carried-forward risk W-2/S-2 per verify-report-2c.md).
...
- [x] T-001 — DONE: Initialize Next.js + TypeScript + Tailwind scaffold
...
- [x] T-002 — DONE: ESLint boundary rules (`eslint-plugin-boundaries`)
...
- [x] T-003 — DONE: `scripts/check-tokens.mjs` raw-hex gate
...
- [x] T-004 — DONE: Design tokens — primitives + semantic layers (light + dark)
...
- [x] T-005 — DONE: Retokenized base component set (shadcn primitives + patterns)
...
- [x] T-006 — DONE: Theme selection (system preference + persisted override)
...
- [x] T-007 — DONE: Mobile-first layout verification harness
...
- [x] T-008 — DONE: PWA shell — manifest + service worker
...
- [x] T-009 — DONE: `core` schema migration — profiles, households, household_members
...
- [x] T-010 — DONE: `core.is_member()` and membership helper functions
...
- [x] T-011 — DONE: RLS policies — `core.*` tables + grants
...
- [x] T-012 — DONE: `core.ensure_personal_space()` — idempotent bootstrap
...
- [x] T-013 — DONE: `app` schema + `app.bootstrap_user()` composition root (core-only step)
...
- [x] T-014 — DONE: Three Supabase clients + middleware session refresh
...
- [x] T-015 — DONE: Google OAuth sign-in flow + callback
...
- [x] T-016 — DONE (written, unexecuted — no local DB access this session): pgTAP — `core` RLS + bootstrap race tests
...
- [x] T-017 — DONE: "No household/hogar in UI" verification checklist + minimal auth shell
...
- [x] T-018 — DONE: `finance.accounts` + detail tables migration
...
- [x] T-019 — DONE: `finance.category_templates` + `finance.categories` migration
...
- [x] T-020 — DONE: `finance.transactions` migration
...
- [x] T-021 — DONE: `finance.account_balances` and `finance.household_summary` views
...
- [x] T-022 — DONE: `finance.ensure_default_categories()` — idempotent per-space seed
...
- [x] T-023 — DONE: `finance.can_read_account()` + RLS policies on `finance.*`
...
- [x] T-024 — DONE: Vitest — pure domain logic unit tests
...
- [x] T-025 — DONE: pgTAP — money invariants (balances, transfers, idempotency, headline split, account creation)
...
- [x] T-026 — DONE: `finance.record_transaction()`, `finance.record_transfer()`, and `finance.create_account()` RPC functions
...
- [x] T-027 — DONE: `finance.update_transaction()` / `finance.update_origin_transaction()` RPC functions
...
- [x] T-028 — DONE: `finance.void_transaction()` and `finance.find_by_origin()` RPC functions
...
- [x] T-029 — DONE: Grant EXECUTE on seam functions; finalize `finance_api.sql` migration
...
- [x] T-030 — DONE: `CREATE OR REPLACE app.bootstrap_user()` to add the Finance step
...
- [x] T-031 — DONE: `src/modules/finance/api/index.ts` — TS facade over the RPC seam
...
- [x] T-032 — DONE: Vitest contract tests — `finance/api` facade against local Supabase
...
- [x] T-033 — DONE: pgTAP — RLS direct-DML-denied regression + definer-recursion-guard
...
- [x] T-034 — DONE: pgTAP — corrections (account move, transfer-leg-reject, void-lock)
...
- [x] T-035 — DONE: pgTAP — category rename/deactivate isolation + idempotent top-up
...
- [x] T-036 — DONE: Minimal account list + creation screen
...
- [x] T-037 — DONE: Minimal transaction entry screen (income/expense/transfer)
...
- [x] T-038 — DONE: Minimal transaction correction affordance
...
- [x] T-039 — DONE: Minimal balance/summary view
...
- [x] T-040 — DONE: E2E smoke suite (Playwright)
  ...
  > remaining Playwright/manual-checklist gap (W-2, S-2) is still open.
  >
  > **Tracker normalization (2026-08-04):** the orchestrator has accepted
  > W-2/S-2 as carried-forward risk (verify-report-2c.md §8) — T-040 is
  > marked `[x]` complete on that basis; the Playwright/manual-checklist
  > gap remains a documented follow-up for a future cycle, not a silent
  > closure.
