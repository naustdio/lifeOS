  ...
  return (
    <div className="flex items-end gap-2">
      <div className="flex w-20 flex-col gap-1">
        <label htmlFor={`ingredientQuantity_${index}`} className="text-xs text-muted-foreground">
          Cant.
        </label>
        <Input
      ...
      <div className="flex w-36 flex-col gap-1">
        <label htmlFor={`ingredientUnit_${index}`} className="text-xs text-muted-foreground">
          Unidad
        </label>
        {customMode ? (
          ...
          <div className="flex items-center gap-1">
            <Select value={unit} onValueChange={(v) => onChange({ unit: v })}>
              <SelectTrigger id={`ingredientUnit_${index}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {units.map((u) => (
      ...
      <div className="flex flex-1 flex-col gap-1">
        <label htmlFor={`ingredientName_${index}`} className="text-xs text-muted-foreground">
          Ingrediente
        </label>
        <Input id={`ingredientName_${index}`} value={name} onChange={(e) => onChange({ name: e.target.value })} required />
      </div>

      <Button type="button" variant="ghost" size="sm" onClick={onRemove} aria-label={`Quitar ingrediente ${index + 1}`}>
